sap.ui.define([
	...
	return Controller.extend("RECI_MASTER.controller.pv_detail", {
		formatter: formatter,
		...
		fnLiveChange: function(oEvent) {
			...
			if (id === "ID_PV_STLAN" || id === "ID_PV_STLAL") {
				...
				if (oBomData && Object.keys(oBomData).length > 0) {
					...
					ErrorHandler.showCustomSnackbar(i18n.getText("MaterialDataClearInfo"), "information");
				}
		...
		fnPvValidation: function() {
			...
			if (!vErrorState) {
				ErrorHandler.showCustomSnackbar(i18n.getText("CorrectHighLightedFieldsError"), "Error");
				busyDialog.close();
				return;
			}
			...
			var oPayload = {
				...
				NavPV_BasicFields: [{
					...
					Mksp: oView.byId("ID_PV_MKSP").getSelectedKey() || oProductionModel.getProperty("/Mksp") || "",
		...
		fnhandleValidationResponse: function(oData, oView) {
			...
			if (aErrorMsgs.length > 0) {
				aErrorMsgs.forEach(function(m) {
					...
					if (sId === "ID_PV_VERID") {
						ErrorHandler.showCustomSnackbar(m.Message, "Error");
					}
				});
		...
		fnSubmitPvData: function() {
			...
			opvModel.NavPV_BasicFields = [{
				...
				Mksp: this.getView().byId("ID_PV_MKSP").getSelectedKey() || ProductionModel.getProperty("/Mksp") || "",
			...
			if (vErrorState) {
				...
				if (pvModel) {
					...
					if (!Array.isArray(ComponentData) || ComponentData.length === 0) {
						ErrorHandler.showCustomSnackbar(i18n.getText("MaterialComponentEmptyError"), "Error");
						return;
					}
				}
		...
		fnOperationDetails: function() {
			...
			if (iSelectedIndex < 0) {
				ErrorHandler.showCustomSnackbar("Please select a material component", "Error");
				return;
			}
			var vTitle = "Choose Operation";

			if (!this.OperFrag) {
				this.OperFrag = sap.ui.xmlfragment(this.getView().getId(), "RECI_MASTER.fragment.OperationList", this);
		...
		fnDeleteFrag: function() {
			...
			if (aSelectedIndices.length === 0) {
				ErrorHandler.showCustomSnackbar(i18n.getText("SelectMaterialComponentError"), "Error");
				return;
			}
			var oPopupModel = new sap.ui.model.json.JSONModel({
				title: "Information",
			...
			if (emptyFields.length > 0) {
				if (emptyFields.length === 1) {
					for (var j = 0; j < emptyFields.length; j++) {
						ErrorHandler.showCustomSnackbar(i18n.getText("MandatoryFieldError", [emptyFields[j].name]), "Error");
					}
				} else {
					ErrorHandler.showCustomSnackbar(i18n.getText("PleaseEnterMandatoryFields"), "Error");
				}
				return false;
			}
			return true;
