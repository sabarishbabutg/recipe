sap.ui.define([
	...
	return Controller.extend("RECI_MASTER.controller.reci_keydata", {
		formatter: formatter,
		...
		fnContinue: function(oEvent) {
			...
			var vProfile = this.getView().byId("KID_RECI_PROFIDNETZ").getValue();
			...
			if (vMaterial === "" && vPlant === "" && vProfile === "" && vReciDes === "") {
				...
				this.getView().byId("KID_RECI_PROFIDNETZ").setValueState("Error");
			...
			if (vPlant === "" || this.getView().byId("KID_RECI_WERKS").getValueState() === "Error") {
				...
				if (this.getView().byId("KID_RECI_WERKS").getValueState() === "Error") {
					...
					ErrorHandler.showCustomSnackbar(valueStateMessage, "Error");
				} else {
					this.getView().byId("KID_RECI_WERKS").setValueState("Error");
					this.getView().byId("KID_RECI_WERKS").setValueStateText("Enter Plant");
				}
			}
			if (vProfile === "" || this.getView().byId("KID_RECI_PROFIDNETZ").getValueState() === "Error") {
				retMsg = false;
				if (this.getView().byId("KID_RECI_PROFIDNETZ").getValueState() === "Error") {
					ErrorHandler.showCustomSnackbar("Please Enter Mandatory Fields", "Error");
					this.getView().byId("KID_RECI_PROFIDNETZ").setValueState("Error");
					this.getView().byId("KID_RECI_PROFIDNETZ").setValueStateText("Enter Valid Profile");
				} else {
					this.getView().byId("KID_RECI_PROFIDNETZ").setValueState("Error");
					this.getView().byId("KID_RECI_PROFIDNETZ").setValueStateText("Enter Profile");
				}
			}
			...
			if (retMsg) {
				...
				var oWFparamModel = this.getOwnerComponent().getModel("JM_WfParm");
				var oWFparamData = oWFparamModel.getData();

				// Deep copy
				oWFparamData = JSON.parse(JSON.stringify(oWFparamData));
				...
				cleanedParamData.AppId = oWFparamData.AppId;
				...
				if (this.Appid !== "RX") {
					...
					this.fnCheckParmChangeProcess().then(function(flag) {
						if (flag) {
							...
							ErrorHandler.showCustomSnackbar("WorkFlow not maintained", "Error");
						}
					}.bind(this));
		...
		fnValidatePlant: function(plant) {
			...
			if (Object.keys(cleanedParamData).length === 0) {
				this.fnSetParmdata(this.Appid);
				var paramData = this.getOwnerComponent().getModel("JM_WfParm").getData();
				Parmmodel = JSON.parse(JSON.stringify(paramData));
			}
		...
		f4descriptionGet: function(vId, value, f4type, fnCallback) {
			...
			filter = [
				...
				new sap.ui.model.Filter("Process", sap.ui.model.FilterOperator.EQ, "K")
			];
		...
		fnPlantF4press: function(oEvent) {
			...
			if (vMaterial === "") {
				...
				this.fnBindFilterTextF4model("P", id, "K", vMaterial, oEvent);
			}
		},
		...
		fnBindFilterTextF4model: function(SearchHelp, sitem, process, material, oEvent) {
			...
			var oPayload = {
				...
				F4Type: SearchHelp,
				FieldNam1: "MATNR",
				Value1: material.toString().padStart(18, "0")
			};
				...
				success: function(oData) {
					...
					if (aResults.length > 0) {
						...
						if (oFirst && (oFirst.DomvalueL || oFirst.Ddtext)) {
							...
							if (this.selectedField === "KID_RECI_PROFIDNETZ") {
								aResults
				...
				success: function(odata, Response) {
					...
					if (aResults.length > 0) {
						...
						if (oFirst && (oFirst.DomvalueL || oFirst.Ddtext)) {
							...
							if (that.selectedField === "KID_RECI_PROFIDNETZ") {
								aResults
									.filter(function(item) {
										return item.Value3 === "C"; // only keep items where Value3 is "C"
									})
			...
			oKeyDataEntity.create("/Recipe_Key_dataSet", Parmmodel, {
				success: function(oData) {
					if (oData.MsgTyp === "S") {
						...
						this.getView().byId("KID_RECI_WERKS").setValueState("Error");
						this.getView().byId("KID_RECI_WERKS").setValueStateText(oData.MsgLine);
						ErrorHandler.showCustomSnackbar(oData.MsgLine, "Error");
						busyDialog.close();
					}
		...
		fnReciDuplicate: function() {
			if (!this.duplicateDialog) {
				this.duplicateDialog = sap.ui.xmlfragment(this.getView().getId(),
					"RECI_MASTER.Fragment.reci_duplicate", // Fragment name
		...
		fnClearallFields: function() {
			...
			this.getView().byId("KID_RECI_WERKS").setValueState("None");
			this.getView().byId("KID_RECI_PROFIDNETZ").setValue("");
			this.getView().byId("KID_RECI_PROFIDNETZ_DES").setValue("");
			this.getView().byId("KID_RECI_PROFIDNETZ").setValueState("None");
			...
			oWFParmSet.read("/WFParmSet", {
				...
				success: function(oData) {
					...
					if (aInvalidParams.length > 0) {
						ErrorHandler.showCustomSnackbar("workflow Param not maintained", "Error");
