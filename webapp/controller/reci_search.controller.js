sap.ui.define([
	...
	return Controller.extend("RECI_MASTER.controller.reci_search", {
		formatter: formatter,
		...
		f4descriptionGet: function(vId, value, f4type, fnCallback) {
			var that = this;
			var filter;
			var oModel = this.getOwnerComponent().getModel("JM_Material");
			...
			filter = [
				...
				new sap.ui.model.Filter("Process", sap.ui.model.FilterOperator.EQ, "S")
			];

			oModel.read("/f4helpSet", {
				filters: filter,
				success: function(oData) {
					that.f4Cache[vId] = oData.results;
					if (fnCallback) {
		...
		bindTextF4model: function(SearchHelp, sitem, process, oEvent) {
			...
			var omodel1 = that.getOwnerComponent().getModel("JM_Material");
			omodel1.read("/f4helpSet", {
				filters: filter,
		...
		fnCollapse: function() {
			this.getView().getModel("JM_Visible").setProperty("/rowCount", 8);
			...
			if (sVarname !== "Select Variant") {
				if (!this.Confirmation) {
					var oPopupModel = new sap.ui.model.json.JSONModel({
						...
						positiveIcon: "Image/Duplicate.svg",
					...
					this.Confirmation = sap.ui.xmlfragment("RECI_MASTER.Fragment.ConfirmationExit", this);
					this.getView().addDependent(this.Confirmation);
				}
				this.Confirmation.open();
			} else {
				if (!this.Variant) {
					this.Variant = sap.ui.xmlfragment("RECI_MASTER.Fragment.create_varient", this);
		...
		fnSubmitConfirmation: function() {
			...
			if (state === "DELETE_VARIENT") {
				...
				if (iIndex === -1) {
					...
					ErrorHandler.showCustomSnackbar(vMsg, "success");
					return;
				}
				...
				oMainModel.create("/Search_VariantSet", oPayload, {
					success: function() {
						ErrorHandler.showCustomSnackbar(i18n.getText("Variant_Delete_Success"), "success");
						this.fnConfirmationFragmentClose();
					}.bind(this),
					error: function() {
						ErrorHandler.showCustomSnackbar(i18n.getText("Variant_Delete_Error"), "Error");
					}
				});
			}
		},
		...
		fnAddVariant: function() {
			...
			if (vMaterial === "" && vPlant === "" && vGroup === "" && vSelectedIndex.length === 14) {
				ErrorHandler.showCustomSnackbar(i18n.getText("inputOrColumnSelectionRequired"), "Warning");
			} else {
				var vTemp = [];
				...
				if (vVariant !== undefined) {
					...
					if (vTemp.length > 0) {
						...
						if (oLast.New === true) {
							ErrorHandler.showCustomSnackbar(i18n.getText("completeSavePreviousRowWarn"), "Warning");
							return;
						}
						var vSelIndex = sap.ui.getCore().byId("id_variantTab").getSelectedIndex();
						if (vSelIndex === -1) {
							ErrorHandler.showCustomSnackbar(i18n.getText("SelectVarientToSave"), "Warning");
							return;
						}
						var vlen = vSelIndex;
				...
				if (vlen >= 0 && vTemp[vlen].VarintName === "") {
					ErrorHandler.showCustomSnackbar(i18n.getText("completePreviousRowWarn"), "Warning");
				} else if (vTextLen < 3) {
					ErrorHandler.showCustomSnackbar(i18n.getText("variantNameMinimumCharsWarn"), "Warning");
				} else if (vVariant !== undefined && this.Save !== undefined && !this.Save) {
					ErrorHandler.showCustomSnackbar(i18n.getText("saveVariantBeforeAddRowWarn"), "Warning");
				} else {
					var vEntity = {
						VarintName: "",
						User: vUser[0].Uname,
		...
		fnDeleteVariant: function() {
			...
			if (vSelIndex < 0) {
				...
				ErrorHandler.showCustomSnackbar(i18n.getText(vErrorMsg, "Error"));
			} else {
				var oPopupModel = new sap.ui.model.json.JSONModel({
					title: "Information",
					...
					positiveIcon: "Image/Continue.svg",
		...
		fnSaveVariantBackend: function(variant) {
			...
			if (exists) {
				...
				ErrorHandler.showCustomSnackbar(i18n.getText("variantAlreadyExists", [variantName]), "Error");
				return; // stop further processing
			}
			// Check if the variant name is at least 4 characters
			if (!variantName || variantName.length < 4) {
				ErrorHandler.showCustomSnackbar(i18n.getText("variantNameMinLength"), "Error");
				return; // stop further processing
			}
			...
			oModel.create("/Search_VariantSet", oEntity, {
				success: function(oData) {
					ErrorHandler.showCustomSnackbar(message, "success");
					if (!variant) {
		...
		fnDownloadTable: function() {
			...
			if (aData.length === 0) {
				ErrorHandler.showCustomSnackbar(i18n.getText("noDatatoDownload"), "Information");
				return;
			}
		...
		fnMenu: function(oEvent) {
			...
			switch (sKey1) {
				...
				case "RECI_CHANGE":
					...
					if (index === -1) {
						ErrorHandler.showCustomSnackbar(i18n.getText("selectRowToChangeProcess"), "Warning");
						return;
					}
					...
					if (this.Selecteddata.Status === "I" || this.Selecteddata.Status === "S" || this.Selecteddata.Status === "D") {
						ErrorHandler.showCustomSnackbar(this.Selecteddata.MsgLine, "Warning");
						return;
					}
		...
		fnSetParmdata: function(AppId) {
			return new Promise(function(Resolve, Reject) {
				var KeyDataId = [
					"KID_RECI_MATNR", "KID_WERKS", "KID_RECI_PROFID_STD", "KID_RECI_RECIDES"
				];
				...
				oWFParmSet.read("/WFParmSet", {
					...
					success: function(oData) {
						...
						if (aInvalidParams.length > 0) {
							ErrorHandler.showCustomSnackbar(i18n.getText("workflowParamNotMaintained"), "Error");
							Reject(false);
		...
		fnSelectBomRow: function(oEvent) {
			...
			if (oContext) {
				...
				if (oRowData.MsgLine !== "") {
					ErrorHandler.showCustomSnackbar(oRowData.MsgLine, vStatus);
				}
			}
		}
	});
