sap.ui.define([
	...
	return Controller.extend("RECI_MASTER.controller.reci_initiator", {
		formatter: formatter,
		...
		fnAttachPress: function() {
			...
			if (iTotalSize > 10 * 1024 * 1024) {
				ErrorHandler.showCustomSnackbar(i18n.getText("totalFileSizeLimit"), "Error");
				return;
			}
			...
			if (bTagExists) {
				ErrorHandler.showCustomSnackbar(i18n.getText("tagNameAlreadyUsed", [sTagNameFinal]), "Error");
				return;
			}
		...
		fnCheckMandatoryFields: function() {
			...
			if (vStatus === "" && vUsage === "") {
				retMsg = false;
				this.fnFocusErroField("ID_RECI_STATU");
				ErrorHandler.showCustomSnackbar(i18n.getText("fillMandatoryField"), "Error");
				this.getView().byId("ID_RECI_STATU").setValueState("Error");
				this.getView().byId("ID_RECI_STATU").setValueStateText(i18n.getText("EnterStatus"));
				this.getView().byId("ID_RECI_VERWE").setValueState("Error");
				...
				retMsg = false;
				this.fnFocusErroField("ID_RECI_STATU");
				ErrorHandler.showCustomSnackbar(i18n.getText("statusMandatory"), "Error");
				this.getView().byId("ID_RECI_STATU").setValueState("Error");
				this.getView().byId("ID_RECI_STATU").setValueStateText(i18n.getText("EnterStatus"));
			} else if (vUsage === "") {
				retMsg = false;
				this.fnFocusErroField("ID_RECI_VERWE");
				ErrorHandler.showCustomSnackbar(i18n.getText("usageMandatory"), "Error");
				this.getView().byId("ID_RECI_VERWE").setValueState("Error");
				this.getView().byId("ID_RECI_VERWE").setValueState(i18n.getText("EnterUsage"));
			}
			return retMsg;
		...
		fnCheckDuplicate: function() {
			...
			oKeyDataEntity.create("/Recipe_Key_dataSet", payLoad, {
				success: function(oData) {
					...
					if (length === 0) {
						ErrorHandler.showCustomSnackbar("No Duplication found", "Information");
						busyDialog.close();
					} else {
			...
			var oHBox = new sap.m.HBox({
				...
				items: [
					new sap.m.HBox({
						...
						items: [
							new sap.m.Image({
								src: "Image/" + ((text === "Recipe Details") ? image.split(" ")[0] : image) + ".svg",
								tooltip: text
		...
		fnReject: function() {
			...
			if (sValue === "") {
				ErrorHandler.showCustomSnackbar(i18n.getText("enterComments"), "Information");
		...
		fnOpenSendBackDialog: function() {
			...
			if (!this.sendBackDialog) {
				this.sendBackDialog = sap.ui.xmlfragment("RECI_MASTER.Fragment.SendBack", this);
		...
		fnsendBack: function() {
			...
			if (!sCurrentLevel) {
				ErrorHandler.showCustomSnackbar(i18n.getText("selectLevelBeforeSubmit"), "Warning");
				busyDialog.close();
				return;
			}
			...
			if (aSelectedAgents.length === 0) {
				ErrorHandler.showCustomSnackbar(i18n.getText("agentSelectionRequired"), "Warning");
				busyDialog.close();
				return;
			}
			var that = this;
			...
			oSendBackService.create("/Recipe_HeaderSet", payload, {
				success: function(oData) {
					if (oData.MsgType === "E") {
						...
						// Show snackbar
						that.fnClearallModel();
						ErrorHandler.showCustomSnackbar(oData.Message, "success");
		...
		fnDraft: function() {
			if (!this.oDialog) {
				...
				// Set model with name
				this.getView().setModel(oPopupModel, "JM_Popup");
				this.oDialog = sap.ui.xmlfragment("RECI_MASTER.Fragment.ConfirmationExit", this);
				this.getView().addDependent(this.oDialog);
			}
			this.oDialog.open();
		},
				...
				oModel.create("/Recipe_HeaderSet", oPayload, {
					success: function(oData) {
						if (oData.MsgTyp !== "E") {
							...
							if (Object.keys(oFromUwl.getData() || {}).length > 0) {
								...
								if (!this.oSuccessdialog) {
									this.oSuccessdialog = sap.ui.xmlfragment(this.getView().getId(),
										"RECI_MASTER.Fragment.SucessDialog", // Fragment path
										this
									);
							...
							ErrorHandler.showCustomSnackbar(message, "Error");
		...
		fnChangelog: function() {
			if (!this.Changelog) {
				this.Changelog = sap.ui.xmlfragment(this.getView().getId(),
					"RECI_MASTER.Fragment.ChangeLog", // Fragment name
					this // Pass controller instance
				);
