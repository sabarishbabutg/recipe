sap.ui.define([
	...
	return Controller.extend("RECI_MASTER.controller.bom_detail", {
		formatter: formatter,
		...
		fnhandleUWLScenario: function(oUwlData, oKeyDataModel, oGobalProductionmodel, oGlobalBomModel) {
			...
			if (oKeyDataModel && oGobalProductionmodel && oGlobalBomModel && oDescModel) {
				...
				this.getView().setModel(new sap.ui.model.json.JSONModel(aDescData), "JM_DescriptionModel");
				this.getView().getModel("JM_DescriptionModel").refresh(true);
				...
				if (oReciBomData && Object.keys(oReciBomData).length > 0) {
		...
		fnsetBOMModels: function(oReciBomData) {
			...
			if (oReciBomData.NavBomItem && oReciBomData.NavBomItem.length > 0) {
				...
				var oDescData = this.getView().getModel("JM_DescriptionModel").getData();
			...
			if (oBomData && Object.keys(oBomData).length === 0) {
				...
				aOdataModel.create("/Recipe_HeaderSet", oPayload, {
					success: function(oData) {
						...
						that.getView().setModel(oDescModel, "JM_DescriptionModel");
						sap.ui.getCore().setModel(oDescModel, "JM_DescriptionModel");

						that.fnBindBomDetails(oData);
					},
					error: function() {
		...
		fnBindBomDetails: function(oData) {
			this.fngetComponent();
						...
						(function(bomItem, compValue) {
							aPromises.push(
								new Promise(function(resolve) {
									that.fnReadf4Cache("ID_BOM_IDNRK", compValue, "P", function(match) {
		...
		fnDeleteFrag: function() {
			...
			if (aSelectedIndices.length === 0) {
				ErrorHandler.showCustomSnackbar(i18n.getText("SelectOneRowToDelete"), "Error");
				return;
			}
		...
		fnRowSelectionChange: function(oEvent) {
			...
			if (oModel) {
				...
				var aSelectedIndices = oTable.getSelectedIndices(); // get currently selected rows
				var aDeselect = [];
					...
					if (shortId.indexOf("ID_BOM_IDNRK") !== -1) {
						...
						if (!vKtext) {
							...
							if (oData && typeof oData === "object") {
								...
								// Update model with a new object reference
		...
		fnReadf4Cache: function(vId, vValue, f4type, callback, oCtx) {
			...
			var updateDesc = function(results) {
				...
				var sKey = vId.replace("ID_BOM_", ""); // e.g., LABOR

				var oModel = that.getView().getModel("JM_DescriptionModel");
				if (oModel) {
		...
		fnBomValidation: function(oEvent, fnSuccessCallback) {
			...
			if (!oBomStatusField || !oBaseQtyField) {
				return;
			}
			...
			var sBomStatus = oBomStatusField.getValue().trim();
			var sBaseQty = oBaseQtyField.getValue().trim();

			if (!sBomStatus || !sBaseQty) {
				if (!sBomStatus) {
					oBomStatusField.setValueState("Error");
					oBomStatusField.setValueStateText("BOM Status is Mandatory");
				}
				if (!sBaseQty) {
					oBaseQtyField.setValueState("Error");
					oBaseQtyField.setValueStateText("Base Quantity is Mandatory");
				}
				ErrorHandler.showCustomSnackbar(i18n.getText("EnterMandatoryFields"), "Error");
				return;
			}
			...
			if (aFilteredItems.length === 0) {
				ErrorHandler.showCustomSnackbar(i18n.getText("EnterComponentDetails"), "Error");
				return;
			}
			...
			oModel.create("/Recipe_HeaderSet", oPayload, {
				...
				error: function() {
					that.NoError = false;
					ErrorHandler.showCustomSnackbar(i18n.getText("ErrorSavingBom"), "Error");
				}
			});
		},
		...
		fnItemDetails: function(oEvent) {
			...
			if (aSelectedIndices.length === 0) {
				ErrorHandler.showCustomSnackbar(i18n.getText("PleaseSelectLineItem"), "Information");
				return;
			}
			...
			if (iSelectedIndex === -1) {
				ErrorHandler.showCustomSnackbar(i18n.getText("PleaseClickSelectedItem"), "Information");
				return;
			}
			...
			// --- Fragment binding ---
			if (!this.clrAllfrag) {
				this.clrAllfrag = sap.ui.xmlfragment("ID_ITEMLEVEL", "RECI_MASTER.Fragment.Itemlevel", this);
			...
			var oDescModel = this.getView().getModel("JM_DescriptionModel");
			...
			if (this.AppId === "RX" && oUwlData.Appid === "RX") {
				...
				if (Array.isArray(aChangeLogData) && aChangeLogData.length > 0) {
					// Filter change log items for current item number and ProcessInd = 'B'
		...
		fnItemSubmit: function() {
			var oView = this.getView();
			var oDescModel = oView.getModel("JM_DescriptionModel");
			...
			if (!oDescModel) {
				...
				oView.setModel(oDescModel, "JM_DescriptionModel");
			}
			...
			if (oBomModel) {
				...
				if (Array.isArray(aBomList)) {
					...
					oView.setModel(oUpdatedDescModel, "JM_DescriptionModel");
					sap.ui.getCore().setModel(oUpdatedDescModel, "JM_DescriptionModel");
				}
			}
		...
		fnSaveBomModels: function() {
			...
			if (BomHeaderModel || BomItemModel) {
				...
				var oExistingLength = aExisting.filter(function(item) {
					return item.Posnr !== "";
				});
				
				if (oExistingLength.length > 0) {
					if (aNew.length > 0) {
						var oNewMap = {};
						aNew.forEach(function(item) {
							oNewMap[item.Idnrk] = item;
						});
						var aUpdated = aExisting.filter(function(item) {
							return oNewMap.hasOwnProperty(item.Idnrk);
						});

						aNew.forEach(function(item) {
							var exists = aUpdated.some(function(existing) {
								return existing.Idnrk === item.Idnrk;
							});
							if (!exists) {
								aUpdated.push({
									Posnr: item.Posnr || "",
		...
		fnNextItem: function() {
			if (this._iCurrentIndex < this._aSelectedPaths.length - 1) {
				...
				ErrorHandler.showCustomSnackbar(i18n.getText("LastLineItemInfo"), "Information");
			}
		},
		...
		fnPreviousItem: function() {
			if (this._iCurrentIndex > 0) {
				...
				ErrorHandler.showCustomSnackbar(i18n.getText("FirstLineItemInfo"), "Information");
			}
		},
		...
		fnDeselectAll: function() {
			...
			if (oTable.getSelectedIndices().length === 0) {
				ErrorHandler.showCustomSnackbar(i18n.getText("NoRowSelected"), "Information");
			}
			oTable.clearSelection();
		},
		...
		fnExpandCollapse: function(oEvent) {
			...
			if (oVisibleModel) {
				...
				oVisibleModel.setProperty("/rowCount", bNew ? 5 : 10);
		...
		fnChangelog: function() {
			if (!this.Changelog) {
				this.Changelog = sap.ui.xmlfragment(this.getView().getId(), "RECI_MASTER.Fragment.ChangeLog", this);
