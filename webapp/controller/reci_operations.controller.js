sap.ui.define([
	...
	return Controller.extend("RECI_MASTER.controller.reci_operations", {
		formatter: formatter,
		...
		fnF4press: function(oEvent) {
			...
			if (this.selectedField.split("-")[0] === "ID_OPER_KTSCH") {
				...
				if (vResource === "") {
					ErrorHandler.showCustomSnackbar(i18n.getText("EnterResource"), "Error");
					return;
				}
			}

			this.bindTextF4model("P", vMaintainedId, "A", oEvent);
		},
		...
		fnSupOperationLiveChange: function(oEvent) {
			...
			if (vValue.length === 4) {
				...
				if (this.operationId.indexOf(vValue) === -1) {
					...
					ErrorHandler.showCustomSnackbar(i18n.getText("enterExistingOperationNumber"), "Error");
			...
			this.fnReadf4Destination("ID_OPER_PHSEQ", vValue, "P").then(function(flag) {
				if (flag) {
					...
					if (vValue.length === 2) {
						...
						ErrorHandler.showCustomSnackbar(i18n.getText("validDestination"), "Error");
						this.fnSetErrorState(sRowPath, sRowPath + "/Phseq");
					}
				}
		...
		fndestinationItemPress: function() {
			...
			if (fromOpeartion === "") {
				...
				ErrorHandler.showCustomSnackbar(i18n.getText("EnterSupOperation"), "Error");
			} else {
		...
		fnSubmitConfirmation: function() {
			...
			if (state === "DELETEROW") {
				...
				if (aSelectedIndices.length === 0) {
					ErrorHandler.showCustomSnackbar(i18n.getText("msgSelectRowToRemove"), "Error");
					return;
				}
				var oModel = this.getView().getModel("JM_ReciOperations");
		...
		fnOperationDialog: function(oEvent) {
			...
			if (aSelectedIndices.length === 0) {
				ErrorHandler.showCustomSnackbar(i18n.getText("SelectRow"), "Information");
				return;
			}
			if (aSelectedIndices.length > 1) {
				ErrorHandler.showCustomSnackbar(i18n.getText("SelectOnlyOneLine"), "Information");
				return;
			}
			...
			if (!oSelectedRow.Arbpl && !oSelectedRow.Pvznr) {
				ErrorHandler.showCustomSnackbar(i18n.getText("SelectOnlyCreatedRecords"), "Information");
				return;
			}
			...
			if (!that.duplicateDialog) {
				that.duplicateDialog = sap.ui.xmlfragment(that.getView().getId(),
					"RECI_MASTER.Fragment.reci_operations", // Fragment name
			...
			oModel.create("/Recipe_HeaderSet", oPayload, {
				success: function(oData) {
					...
					if (oData.NavReturn_Msg && oData.NavReturn_Msg.results && oData.NavReturn_Msg.results.length > 0 && first !== "S") {
						...
						ErrorHandler.showCustomSnackbar(sFirstMsg, "Error");
		...
		fnApplyCustomizeColumn: function() {
			...
			if (vAllFalse === true) {
				ErrorHandler.showCustomSnackbar(i18n.getText("ColumnSelectionMandatory"), "Warning");
		...
		fnDeselectAll: function() {
			...
			if (oTable.getSelectedIndices().length === 0) {
				ErrorHandler.showCustomSnackbar("No row selected", "Warning");
			}
			oTable.clearSelection();
		},
		...
		fnDeleteRow: function(oEvent) {
			...
			indexValue.push(aSelectedIndices);
			if (aSelectedIndices.length === 0) {
				ErrorHandler.showCustomSnackbar("Please select a row to remove", "Error");
				return;
			}
			var oPopupModel = new sap.ui.model.json.JSONModel({
				title: i18n.getText("Information"),
			...
			serviceCall.create("/Recipe_HeaderSet", oPayload, {
				success: function(oData) {
					if (oData.MsgTyp === "E") {
						...
						ErrorHandler.showCustomSnackbar(ErrorPopUp, "Error");
